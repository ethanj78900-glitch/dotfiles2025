...
kvantum:
	@echo "************ kvantum setup ************"
	@mkdir -p ${HOME}/.config
	@if [ -d kvantum/.git ]; then \
	  git submodule init kvantum && \
	  git submodule update kvantum --recursive; \
	else \
	  echo "Skipping git submodule (kvantum not a submodule)"; \
	fi
	@stow -S kvantum -t ${HOME} 2>/dev/null || echo "Skipping stow (kvantum may not have a valid stow structure)"
	@echo ""
